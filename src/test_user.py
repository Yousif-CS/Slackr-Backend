...
def test_user_profile_setname():
    user_profile_setname(user_kli["token"], "Kenneth", "Lithium")
    assert user_profile(user_kli["token"], user_kli["u_id"]) == \
        {"user": {
        	"u_id": user_kli["u_id"],
        	"email": "ken@gmail.com",
...
def test_user_profile_setname_invalid():
    with pytest.raises(InputError):
        user_profile_setname(user_kli["token"], "", "Lithium")
        user_profile_setname(user_kli["token"], "Kenneth", "")
        user_profile_setname(user_kli["token"], "K" * 51, "Lithium")
        user_profile_setname(user_kli["token"], "Kenneth", "L" * 51)
        user_profile_setname(user_kli["token"], "       ", "Li")
        user_profile_setname(user_kli["token"], "Ken", "        ")
