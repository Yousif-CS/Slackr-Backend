...
# check type of message_id to be int and that user messages' message_id's
# are unique
...
# testing that identical messages (messages with identical strings) are
# processed correctly
...
# 1. messages with white space and symbols, as long as length is under
# 1000 characters
...
# testing for AccesError: when user of channel leaves the channel and
...
# testing for Exception: when channel_id is invalid and the channel does
# not actually exist
...
# testing that a member of channel added by the owner is able to remove
# their own message
...
def test_message_remove_user_remove_msg(make_users):
    ...
    # checking that AccessError will be thrown when user_cd tries to delete
    # either message
    with pytest.raises(AccessError):
...
# when an owner leaves a channel, they can no longer edit messages in that
# channel
...
# AccessError: auth_user is not a member of the channel which contains the
# message
...
# InputError: check that attempting to react to a nonexistent message
# throws exception
...
# InputError: user has already reacted with the react ID to the message
# with message_id
def test_message_react_twice(make_users):
    ...
    # check that InputError is thrown when user_ab tries to react again with
    # same react_id
    with pytest.raises(InputError):
        message_react(user_ab['token'], msg1['message_id'], 1)
...
# check that when a user unreacts, they are no longer part of the list of
# reacted users for that msg
...
def test_message_unreact_standard(make_users):
    ...
    # check that the list of reacted users is now empty and is_user_reacted
    # set to False
    assert messages_public[0]['reacts'] == [{
        'react_id': 1,
        'u_ids': [],
...
# check that when multiple users have reacted and then one unreacts, the
# other one still remains
...
# InputError: user that left channel cannot unreact to any messages (incl.
# their own)
...
# InputError: user cannot unreact if they did not already react with the
# same id
