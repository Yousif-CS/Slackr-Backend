...
def test_message_send_not_part_of_channel(make_users):
    # setting up users and public channel
    user_ab, user_cd = make_users
    new_private_channel = channels_create(user_ab['token'], 'test_channel_public', False)
...
def test_message_send_invalid_token(make_users):
    ...
    with pytest.raises(AccessError):
        message_send(user_ab['token']+ "invalid", new_public_channel['channel_id'], "Invalid token")
...
def test_message_remove_owner_remove_own(make_users):
    ...
    msg1 = message_send(user_ab['token'], new_public_channel['channel_id'], "Hello world!")
    msg2 = message_send(user_ab['token'], new_public_channel['channel_id'], "Second message")
...
def test_message_remove_user_remove_own(make_users):
    ...
    msg1 = message_send(user_ab['token'], new_private_channel['channel_id'], "Please do not post")
...
def test_message_edit_owner_left_channel_invalid():
    # setting up users and public channel
    workspace_reset
    user_zero = auth_register("admin@gmail.com", "admin1", "Admin", "Slackr_Owner")
    ...
    msg_id = message_send(user_cd["token"], new_ch["channel_id"], "Second owner message")["message_id"]
